...
glm::mat4 Matrix_Rotate(float angle, glm::vec4 axis)
{
    ...
    return Matrix(
        ...
        vx*vx*(1-c) + c , vx*vy*(1-c) - vz*s , vx*vz*(1-c) + vy*s , 0 ,  // LINHA 1
        vx*vy*(1-c) + vz*s , vy*vy*(1-c) + c , vy*vz*(1-c) - vx*s , 0 ,  // LINHA 2
        vx*vz*(1-c) - vy*s , vy*vz*(1-c) + vx*s , vz*vz*(1-c) + c , 0 ,  // LINHA 3
        0 , 0 , 0 , 1    // LINHA 4
    );
}
